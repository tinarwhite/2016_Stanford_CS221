...
from cs231n.rom_utils import sample_burg_minibatch
...
class PDESolver(object):
  ...
  def _step(self):
    ...
    minibatch = sample_burg_minibatch(self.data,
                  batch_size=self.batch_size)
    solutions, features = minibatch

    # Compute loss and gradient
    loss, grads = self.model.loss(features, solutions)
  ...
  def train(self):
    ...
    """
    num_train = self.data['solutions'].shape[0]
...
class Solver(object):
  ...
  def check_predictions(self, X, y, num_samples=None, batch_size=100):
    """
    ...
    sample = X_batch
    sample_predictions = scores
    return sample, sample_predictions
  ...
  def check_accuracy(self, X, y, num_samples=None, batch_size=100):
    ...
    for i in xrange(num_batches):
      ...
      #print N, batch_size, X[start:end].shape, y[start:end].shape
      loss, grads = self.model.loss(X[start:end], y[start:end])
      y_pred.append(loss)
    y_pred = np.hstack(y_pred)
    acc = np.mean(y_pred)
  ...
  def train(self):
    ...
    for t in xrange(num_iterations):
      ...
      if first_it or last_it or epoch_end:
        train_acc = self.check_accuracy(self.X_train, self.y_train, self.batch_size)
        val_acc = self.check_accuracy(self.X_val, self.y_val, self.batch_size)
      ...
      if t == num_iterations-2:
        self.sample, self.sample_predictions = self.check_predictions(self.X_val, self.y_val, self.batch_size)
